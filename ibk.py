...
main_application.title('ibk text editor')
...
canvas_color = tk.Menu(main_menu, tearoff=False)

theme_choice = tk.StringVar()
color_icons = (light_default_icon, light_plus_icon, dark_icon,
               pink_icon, monokai_icon, night_blue_icon)
...
main_menu.add_cascade(label='Canvas Color', menu=canvas_color)
...
font_box.current(font_tuple.index('Georgia'))
...
# font family ad size functionality
current_font_family = 'Georgia'
...
text_editor.configure(font=('Georgia', 12))
### end text editor ###
...
edit.add_command(label='Clear All', image=clear_all_icon,
                 compound=tk.LEFT, accelerator='Ctrl+Alt+X', command=lambda: text_editor.delete(1.0, tk.END))
...
view.add_checkbutton(label='Tool Bar', onvalue=True,
                     offvalue=0, variable=show_toolbar, image=tool_bar_icon, compound=tk.LEFT, command=hide_toolbar)
